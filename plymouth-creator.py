...
class Handler:
    ...
    def on_selectVideoButton_clicked(button):
        ...
        videoInputFile = askopenfilename(
            parent=root,
            initialdir="/home",
            title="Select your .mp4 video",
    ...
    def on_selectVideoOutputButton_clicked(button):
        ...
        videoOutputDirectory = askdirectory(
            parent=root, initialdir="/home", title="Select an empty output directory"
        )
    ...
    def on_selectDirectoryButton_clicked(button):
        ...
        outputDirectory = askdirectory(
            parent=root, initialdir="/home", title="Select directory with .png files"
        )
    ...
    def on_generateButton_clicked(button):
        ...
        if setOutputDirectory == True:
            ...
            plymouthFile = open(outputDirectory + "/" +
                                name + ".plymouth", "w")
            ...
            scriptFile.write(
                "for(i = 1; i <= " + str(amountOfPngFiles) + "; i++)\n")
            scriptFile.write("{\n")
            if scaleImages == False:
                ...
                scriptFile.write(
                    '  image[i].image = Image("image-" + i + ".png").Scale(Window.GetWidth(), Window.GetHeight());\n'
                )
            ...
            scriptFile.write("{\n")
            scriptFile.write(
                "  if(index >= " + str(amountOfPngFiles) + " + 1)\n")
            scriptFile.write("  {\n")
            if loopAnimation == False:
                scriptFile.write(
                    "    image[" + str(amountOfPngFiles) +
                    "].sprite.SetOpacity(1);\n"
                )
            else:
                scriptFile.write("    index = 1;\n")
            ...
            installFile.write(
                "sudo mkdir /usr/share/plymouth/themes/" + name + "\n")
            installFile.write(
                "sudo cp -rf ./ /usr/share/plymouth/themes/" + name + "\n"
            )
            installFile.write(
                "sudo update-alternatives --quiet --install /usr/share/plymouth/themes/default.plymouth default.plymouth /usr/share/plymouth/themes/"
                + name
                + "/"
                + name
                + ".plymouth 100\n"
            ...
            installFile.write(
                "sudo update-alternatives --quiet --set default.plymouth /usr/share/plymouth/themes/"
                + name
                + "/"
                + name
                + ".plymouth\n"
            ...
            uninstallFile.write(
                "echo Uninstalling the " + name + " theme...\n")
            uninstallFile.write(
                "sudo update-alternatives --quiet --remove default.plymouth /usr/share/plymouth/themes/"
                + name
                + "/"
                + name
                + ".plymouth\n"
            ...
            uninstallFile.write(
                "sudo rm -rf /usr/share/plymouth/themes/" + name + "\n")
            uninstallFile.write(
